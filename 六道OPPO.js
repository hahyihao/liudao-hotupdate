...
var __LIUDAO_IS_OPPO_BUILD = true;
...
/** OPPO / ColorOS / 一加 / Realme（悬浮窗与无障碍长跑更易卡死） */
function isOppoColorOsDevice() {
  var parts = [];
  try { parts.push(String(device.brand || "").trim().toLowerCase()); } catch (e0) {}
  try { parts.push(String(device.manufacturer || "").trim().toLowerCase()); } catch (e1) {}
  try { parts.push(String(device.product || "").trim().toLowerCase()); } catch (e2) {}
  var blob = parts.join("|");
  return /oppo|oneplus|realme|coloros|heytap|oplus/i.test(blob);
}

function getDeviceIdentityBlob() {
  var parts = [];
  try { parts.push(String(device.brand || "").trim().toLowerCase()); } catch (e0) {}
  try { parts.push(String(device.manufacturer || "").trim().toLowerCase()); } catch (e1) {}
  try { parts.push(String(device.model || "").trim().toLowerCase()); } catch (e2) {}
  try { parts.push(String(device.product || "").trim().toLowerCase()); } catch (e3) {}
  return parts.join("|");
}

/** 华为 Mate 9 等老机型：进主页渲染慢，第10步须多点一次「作品」并加长等待 */
function isHuaweiMate9Device() {
  var blob = getDeviceIdentityBlob();
  return /mate\s*9|mha-al00|mha-tl00|mha-cl00|mha-ll00|mha-ul00/i.test(blob);
}

/**
 * Reno5 / R17 / 老 ColorOS：关闭深度恢复（强打断、误重建），保留基础加固与悬浮窗补建。
 * 注意：Reno3（PCLM50）不入此名单——长跑易出「僵尸悬浮窗」，需要定期重建。
 */
function isOppoColorOsLightHardeningOnly() {
  if (!isOppoColorOsDevice()) return false;
  var blob = getDeviceIdentityBlob();
  return /reno5|pdsm00|pdst00|pdpm00|pdam00|cph2201|cph2145|cph2159|cph2205|cph2219|r17|pbet00|pbem00|pbet|pbcmt00|pbdm00|cph1881|cph1897|cph1851/i.test(
    blob
  );
}

/**
 * ColorOS 上易出现「Java 对象仍可 setText/getX，但画面与触摸已死」的僵尸悬浮窗。
 * Reno3 元气版（PCLM50）等机型长跑后尤甚，仅靠读屏无法判定，须定期强制重建。
 */
function isOppoZombieFloatProneDevice() {
  if (!isOppoColorOsDevice()) return false;
  var blob = getDeviceIdentityBlob();
  return /reno3|pclm50|pclm10|reno4|pdpm00|a72|cph2067|cph2069|cph2065|reno/i.test(blob);
}

/** 是否启用 OPPO 深度恢复（强打断/悬浮窗重建/更短卡住阈值）；轻量机型默认关闭 */
function isOppoAggressiveRecoveryEnabled() {
  if (!isOppoColorOsDevice()) return false;
  if (isOppoColorOsLightHardeningOnly()) return false;
  return true;
}
...
function readFloatClockTextFromWindow() {
  if (!__floatInfoWin || !__floatInfoWin.line_12) return "";
  try {
    return String(__floatInfoWin.line_12.getText() || "").trim();
  } catch (e) {
    return "";
  }
}

function parseFloatClockDisplayMinuteTs(text) {
  if (!text) return 0;
  var m = String(text)
    .trim()
...
/** 悬浮窗时间行相对真实时间落后多少毫秒（读屏校验，非 setText 返回值） */
function getFloatClockDisplayLagMs() {
  var shown = readFloatClockTextFromWindow();
  if (!shown) return 999999;
  var expected = formatFloatNowDateTimeZh();
  if (shown === expected) return 0;
  var shownTs = parseFloatClockDisplayMinuteTs(shown);
  var expectedTs = parseFloatClockDisplayMinuteTs(expected);
  if (!shownTs || !expectedTs) return shown === expected ? 0 : __OPPO_FLOAT_CLOCK_STALE_MS;
  return Math.max(0, expectedTs - shownTs);
}

function markFloatClockUiGood() {
  var now = Date.now();
  __floatClockUiAppliedMs = now;
  __floatClockLastGoodMs = now;
}

function applyFloatClockLineVerified(win, text) {
  if (!win || !win.line_12 || !text) return false;
  try {
    win.line_12.setText(text);
    if (readFloatClockTextFromWindow() === text) {
      markFloatClockUiGood();
      return true;
    }
  } catch (e0) {}
  return false;
...
function isFloatUiDisplayFrozen() {
  if (!__floatCreated || __scriptUserStop || !__automationWorkerActive) return false;
  if (!probeFloatWindowResponsive()) return true;
  if (getFloatClockDisplayLagMs() >= __OPPO_FLOAT_CLOCK_STALE_MS) return true;
  if (
    __floatClockUiRequestedMs > 0 &&
    __floatClockLastGoodMs > 0 &&
    Date.now() - __floatClockLastGoodMs > __OPPO_FLOAT_UI_DEAD_MS
  ) {
    return true;
  }
  return false;
...
/** 悬浮窗画面冻结或丢失时重建（满额等零点也生效，不依赖深度恢复开关） */
function maybeRecoverFrozenFloatWindow(reason) {
  if (!isOppoColorOsDevice()) return false;
  if (!__floatCreated || __scriptUserStop || !__automationWorkerActive) return false;
  if (__oppoFloatCountdownActive) return false;
  if (!isFloatUiDisplayFrozen()) return false;
  var now = Date.now();
  if (now - __oppoLastFloatRecreateMs < __OPPO_FLOAT_RECREATE_COOLDOWN_MS) return false;
  __oppoLastFloatRecreateMs = now;
  var revived = false;
  try {
    revived = !!recreateFloatWindowSafe(reason || "悬浮窗显示冻结重建");
  } catch (eRf) {}
  if (revived) {
    markFloatClockUiGood();
    __oppoLastProactiveFloatRecreateMs = Date.now();
  }
  return revived;
}

/**
 * ColorOS 僵尸悬浮窗：对象仍可 setText/getX，但画面/触摸已死，读屏判不出来。
 * 定时强制拆掉重建，让 × 和时钟重新可用（Reno3 等机型尤甚）。
 */
function maybeProactiveRecreateFloatWindow() {
  if (!isOppoColorOsDevice()) return false;
  if (!__floatCreated || __scriptUserStop || !__automationWorkerActive) return false;
  if (__oppoFloatCountdownActive) return false;
  var interval = isOppoZombieFloatProneDevice()
    ? __OPPO_FLOAT_PROACTIVE_RECREATE_ZOMBIE_MS
    : __OPPO_FLOAT_PROACTIVE_RECREATE_MS;
  var now = Date.now();
  if (!__oppoLastProactiveFloatRecreateMs) {
    __oppoLastProactiveFloatRecreateMs = now;
    return false;
  }
  if (now - __oppoLastProactiveFloatRecreateMs < interval) return false;
  if (now - __oppoLastFloatRecreateMs < __OPPO_FLOAT_RECREATE_COOLDOWN_MS) return false;
  __oppoLastFloatRecreateMs = now;
  __oppoLastProactiveFloatRecreateMs = now;
  var ok = false;
  try {
    ok = !!recreateFloatWindowSafe("OPPO悬浮窗预防性重建");
  } catch (ePr) {}
  if (ok) markFloatClockUiGood();
  return ok;
}

/** 连按音量减 3 次：紧急停止（悬浮窗 × 卡死时的出路） */
function startOppoEmergencyStopKeyOnce() {
  if (__oppoEmergencyStopKeyStarted) return;
  if (!isOppoColorOsDevice()) return;
  __oppoEmergencyStopKeyStarted = true;
  try {
    events.observeKey();
    events.on("key_down", function (keyCode, event) {
      try {
        if (__scriptUserStop) return;
        if (keyCode !== keys.volume_down) return;
        var now = Date.now();
        if (now - __oppoVolDownTapLastMs > 1800) __oppoVolDownTapCount = 0;
        __oppoVolDownTapLastMs = now;
        __oppoVolDownTapCount++;
        if (__oppoVolDownTapCount >= 3) {
          __oppoVolDownTapCount = 0;
          try {
            toast("音量键紧急停止");
          } catch (eT) {}
          requestStopFromFloatCloseTouch();
        }
      } catch (eKey) {}
    });
  } catch (eObs) {
    __oppoEmergencyStopKeyStarted = false;
  }
}
...
function updateFloatClockLine() {
  ...
  __floatClockUiRequestedMs = Date.now();
  if (isOppoColorOsDevice()) {
    if (applyFloatClockLineVerified(win, text)) {
      if (win.line_13 && __automationWorkerActive) {
        try {
          win.line_13.setText(
    ...
    if (probeFloatWindowResponsive()) {
      try {
        ui.run(function () {
          try {
            if (win && win.line_12) win.line_12.setText(text);
            if (win && win.line_13 && __automationWorkerActive) {
              win.line_13.setText(
                "已运行" + formatRunDurationHoursMinutesZh(getRunTimeMsTodayForDisplay())
              );
            }
...
try {
  if (isOppoColorOsDevice()) {
    threads.start(function () {
      sleep(600);
      handleOppoAppListPermissionDialogIfAny(15000);
    });
  }
} catch (eOppoAppListInit) {}
...
function updateAppVersionTitleDisplay() {
  ...
  try {
    ...
    if (isOppoColorOsDevice() && __floatInfoWin && __floatInfoWin.line_1) {
      try {
        __floatInfoWin.line_1.setText(title);
      } catch (eFloatDirect) {}
    }
...
/** OPPO 专项：日志落盘、看门狗硬恢复、悬浮窗/UI 存活检测 */
var __LOG_PERSIST_KEY = "automation_log_snapshot";
var __LOG_PERSIST_TS_KEY = "automation_log_snapshot_ts";
var __logPersistPending = false;
var __skipFloatUiFlush = false;
var __floatClockUiRequestedMs = 0;
var __floatClockUiAppliedMs = 0;
/** 悬浮窗时间行最后一次「读回校验通过」的时刻（区分 setText 成功与画面真刷新） */
var __floatClockLastGoodMs = 0;
/** 悬浮窗显示分钟落后真实时间超过此值视为卡死（ms） */
var __OPPO_FLOAT_CLOCK_STALE_MS = 60000;
/** 悬浮窗冻结重建冷却（ms） */
var __OPPO_FLOAT_RECREATE_COOLDOWN_MS = 8000;
/** ColorOS 僵尸窗：预防性重建周期（普通 OPPO） */
var __OPPO_FLOAT_PROACTIVE_RECREATE_MS = 12 * 60 * 1000;
/** Reno3 等易僵机型：更短预防重建周期 */
var __OPPO_FLOAT_PROACTIVE_RECREATE_ZOMBIE_MS = 8 * 60 * 1000;
var __oppoLastProactiveFloatRecreateMs = 0;
var __oppoEmergencyStopKeyStarted = false;
var __oppoVolDownTapCount = 0;
var __oppoVolDownTapLastMs = 0;
var __quotaIdleLoopTick = 0;
var __oppoHardRecoveryInProgress = false;
var __oppoLastHardRecoveryAttemptMs = 0;
var __oppoLastFloatRecreateMs = 0;
var __oppoWatchdogSec = 0;
var __OPPO_HEARTBEAT_EVERY_SEC = 120;
var __OPPO_FLOAT_UI_DEAD_MS = 45000;
var __OPPO_HARD_RECOVERY_WAIT_MS = 800;
var __oppoAutomationKeepAwakeActive = false;
var __oppoKeepAwakeRefreshTick = 0;
var __OPPO_KEEP_AWAKE_REFRESH_EVERY = 60;
var __stuckRestartStartedMs = 0;
var __oppoLastNuclearUnstickMs = 0;
var __oppoLastQueryTimeoutLogMs = 0;
var __oppoFloatCountdownActive = false;
var __oppoLastA11yEmptyLogMs = 0;
var __RUN_HEARTBEAT_TS_KEY = "automationRunHeartbeatTs";
var __RUN_CLEAN_STOP_KEY = "automationRunCleanStop";
...
try {
  if (isOppoColorOsDevice()) restoreAutomationLogSnapshot();
} catch (eLogRestoreInit) {}
...
function sleepCtrlCountdown(label, ms) {
  ...
  var lastShown = -1;
  __floatCountdownLogIdx = -1;
  __oppoFloatCountdownActive = true;
  try {
    ...
    __oppoFloatCountdownActive = false;
...
/** 点 × 时在独立线程关脚本，避免 UI 线程卡死时关不掉 */
function requestStopFromFloatCloseTouch() {
  if (__scriptUserStop) return;
  __scriptUserStop = true;
  threads.start(function () {
    try {
...
function stopEntireScriptAfterClosingFloat() {
  __scriptUserStop = true;
  try {
    markAutomationRunCleanStop();
  } catch (eMk) {}
  try { endQuotaIdleKeepAwake(); } catch (eQa) {}
  try { endOppoAutomationKeepAwake(); } catch (eOa) {}
  try { persistAutomationLogSnapshot(true); } catch (eLp) {}
  try {
...
function updateFloatInfo() {
  ...
  if (isOppoColorOsDevice() && __automationWorkerActive) {
    try {
      if (updateFloatInfoDirect()) return;
    } catch (eDir) {}
  }
  ensureTaskCountDayRolled();
...
/** OPPO 自动化：不经 ui.run 直接刷悬浮窗，避免 UI 线程死锁导致时间不走 */
function updateFloatInfoDirect() {
  if (!__floatInfoWin) return false;
  ensureTaskCountDayRolled();
  ...
  var total = isSoda ? like + comment : like + fav + comment + share;
  var done = getTaskDoneSnapshot();
  var sumDone = isSoda
    ? done.like + done.comment
    : done.like + done.fav + done.comment + done.share;
  var start = Math.max(0, __logLines.length - __logShowLines);
  var logText = __logLines.slice(start).join("\n");
  var w = __floatInfoWin;
  var nick = __nickname || "您的昵称";
  var fans = __fans || "0";
  var yest = __yesterdayGrowth;
  var today = __todayGrowth;
  var plat = getPlatformLabel();
  var clockStr = formatFloatNowDateTimeZh();
  var runTodayStr =
    "已运行" + formatRunDurationHoursMinutesZh(getRunTimeMsTodayForDisplay());
  try {
    if (w.line_2) w.line_2.setText(nick);
    ...
    if (w.line_13) w.line_13.setText(runTodayStr);
    if (w.log) w.log.setText(logText);
    if (readFloatClockTextFromWindow() === clockStr) markFloatClockUiGood();
    return true;
  } catch (e0) {
    return false;
  }
...
function isAccessibilityServiceAlive() {
  try {
...
/** OPPO 深度恢复：仅记录无障碍对象是否为空（不探测、不 waitFor、不强打断，避免误伤 Reno5 等） */
function maybeOppoAccessibilityWatchdog() {
  if (!isOppoAggressiveRecoveryEnabled() || !__automationWorkerActive || __scriptUserStop) return;
  if (isAccessibilityServiceAlive()) return;
  var now = Date.now();
  if (now - __oppoLastA11yEmptyLogMs < 60000) return;
  __oppoLastA11yEmptyLogMs = now;
  try {
    appendWatchdogDiagLog("无障碍对象为空");
  } catch (e0) {}
}

function dyUiFindOne(selectorFn, oppoMaxMs) {
  if (!isOppoColorOsDevice()) {
    try {
      return selectorFn();
    } catch (e0) {
  ...
  var maxMs = oppoMaxMs > 0 ? oppoMaxMs : 2000;
  var out = null;
  var done = false;
  var th = null;
  try {
  ...
  while (!done && Date.now() < deadline) sleep(25);
  if (!done) {
    try {
      th.interrupt();
    } catch (e3) {}
...
function shouldOppoAutomationAsyncLogOnly() {
  return isOppoColorOsDevice() && __automationWorkerActive && !__skipFloatUiFlush;
}

/** 仅刷新「过久无日志」计时，不打字（避免与 appendLog 重复计同一文案触发卡住重启） */
function bumpStuckWatchdogHeartbeat() {
  try {
    // 满额等零点：主循环会周期性调用本函数，不应掩盖悬浮窗/UI 卡死检测
    if (__quotaIdleKeepAwakeActive) return;
    __lastAppendLogTimeMs = Date.now();
  } catch (eHb) {}
}

function markAutomationRunStarted() {
  try {
    __stats.put(__RUN_CLEAN_STOP_KEY, false);
    __stats.put(__RUN_HEARTBEAT_TS_KEY, Date.now());
  } catch (e) {}
}

function markAutomationRunCleanStop() {
  try {
    __stats.put(__RUN_CLEAN_STOP_KEY, true);
  } catch (e) {}
}

function persistAutomationRunHeartbeat() {
  if (!__automationWorkerActive || __scriptUserStop) return;
  try {
    __stats.put(__RUN_HEARTBEAT_TS_KEY, Date.now());
    __stats.put(__RUN_CLEAN_STOP_KEY, false);
  } catch (e) {}
...
/** OPPO：若上次未正常收尾就断了，下次启动时提示（多为系统杀进程） */
function checkAndLogAbnormalLastAutomationExit() {
  if (!isOppoColorOsDevice()) return;
  try {
    var clean = __stats.get(__RUN_CLEAN_STOP_KEY, true);
    if (clean === true || clean === "true" || clean === 1 || clean === "1") return;
    var ts = Number(__stats.get(__RUN_HEARTBEAT_TS_KEY, 0) || 0);
    if (!ts) return;
    var ago = Date.now() - ts;
    if (ago > 0 && ago < 6 * 3600 * 1000) {
      var agoMin = Math.max(1, Math.floor(ago / 60000));
      appendWatchdogDiagLog("上次运行约" + agoMin + "分钟前异常退出(进程可能被系统结束)");
    }
  } catch (e) {}
}

function runOnUiThreadSync(fn, maxMs) {
  maxMs = maxMs > 0 ? maxMs : 3500;
  var done = false;
  try {
    ui.run(function () {
      try {
...
function isFloatWindowPairAlive() {
  return probeFloatWindowResponsive();
}

/** 悬浮窗是否仍可响应（区分「变量还在」与「已被系统回收」） */
function probeFloatWindowResponsive() {
...
/** 所有 OPPO：窗体丢失时补建（不依赖深度恢复开关） */
function ensureFloatWindowIfMissing(reason) {
  if (!__floatCreated || __scriptUserStop || !isOppoColorOsDevice()) return false;
  if (!__automationWorkerActive) return false;
  if (probeFloatWindowResponsive()) return true;
  var now = Date.now();
  if (now - __oppoLastFloatRecreateMs < 5000) return false;
  __oppoLastFloatRecreateMs = now;
  return recreateFloatWindowSafe(reason || "OPPO悬浮窗补建");
}

function persistAutomationLogSnapshot(forceNow) {
  if (!isOppoColorOsDevice()) return;
  if (!forceNow) {
    if (__logPersistPending) return;
    __logPersistPending = true;
    try {
      threads.start(function () {
        sleep(350);
        __logPersistPending = false;
        try {
          __stats.put(__LOG_PERSIST_KEY, JSON.stringify(__logLines.slice(-__logMaxLines)));
          __stats.put(__LOG_PERSIST_TS_KEY, Date.now());
        } catch (e0) {}
      });
    } catch (e1) {
      __logPersistPending = false;
...
function restoreAutomationLogSnapshot() {
  if (!isOppoColorOsDevice()) return false;
  try {
    var raw = __stats.get(__LOG_PERSIST_KEY, "");
    if (!raw) return false;
    var arr = JSON.parse(String(raw));
    if (!Array.isArray(arr) || !arr.length) return false;
    __logLines = arr.slice(-__logMaxLines);
    return true;
  } catch (e) {
    return false;
  }
...
/** 看门狗专用：写内存+console+落盘，不刷新悬浮窗、不重置卡住计时 */
function appendWatchdogDiagLog(msg) {
  var s = normalizeLogText(msg);
  if (!s) return;
  pushLogLine(s);
  persistAutomationLogSnapshot(false);
}

function beginOppoAutomationKeepAwake() {
  if (!isOppoColorOsDevice()) return;
  if (__oppoAutomationKeepAwakeActive) return;
  __oppoAutomationKeepAwakeActive = true;
  __oppoKeepAwakeRefreshTick = 0;
  try {
...
function refreshOppoAutomationKeepAwake() {
  if (!__oppoAutomationKeepAwakeActive) return;
  __oppoKeepAwakeRefreshTick++;
  if (__oppoKeepAwakeRefreshTick % __OPPO_KEEP_AWAKE_REFRESH_EVERY !== 0) return;
  try {
    device.keepScreenDim();
  } catch (e) {}
}

function endOppoAutomationKeepAwake() {
  if (!__oppoAutomationKeepAwakeActive) return;
  __oppoAutomationKeepAwakeActive = false;
  __oppoKeepAwakeRefreshTick = 0;
  if (!__quotaIdleKeepAwakeActive) {
    try {
...
/**
 * OPPO：无障碍 .find() 可能永久阻塞，放入子线程并限时放弃。
 * @param {number} maxMs
 * @param {function():*} fn 返回 UiCollection
 * @returns {Array}
 */
function oppoBoundedUiQuery(maxMs, fn) {
  maxMs = maxMs > 0 ? maxMs : 2000;
  var out = [];
  var done = false;
  var th = null;
  try {
  ...
  if (!done) {
    ...
    var nowTo = Date.now();
    if (nowTo - __oppoLastQueryTimeoutLogMs > 30000) {
      __oppoLastQueryTimeoutLogMs = nowTo;
      try {
        appendLogFast("OPPO节点查询超时");
      } catch (e4) {}
    }
  }
  return out;
...
/** 抖音粉丝列表等：非 OPPO 直接 find；OPPO 限时 find */
function dyUiFindCollection(selectorFn, oppoMaxMs) {
  if (!isOppoColorOsDevice()) {
    try {
      return uiCollectionToArray(selectorFn());
    } catch (e0) {
      return [];
    }
  }
  return oppoBoundedUiQuery(oppoMaxMs || 2000, selectorFn);
}

function recreateFloatWindowSafe(reason) {
  if (!__floatCreated || __scriptUserStop) return false;
  try {
    appendWatchdogDiagLog(reason || "OPPO悬浮窗重建");
  } catch (e0) {}
  var oldX = __floatPanelX;
  var oldY = __floatPanelY;
  try {
    if (__floatCtrlWin) {
      try {
  ...
  // ColorOS 僵尸层：尽量关掉全部 floaty，避免旧窗盖在新窗上面、触摸进不去
  try {
    if (typeof floaty !== "undefined" && floaty.closeAll) floaty.closeAll();
  } catch (eCloseAll) {}
  var recreated = false;
  for (var attempt = 0; attempt < 3 && !recreated; attempt++) {
    if (attempt > 0) sleep(350 + attempt * 200);
    try {
      runOnUiThreadSync(function () {
        __floatPanelX = oldX;
        __floatPanelY = oldY;
        createOrShowFloatWindow();
        updateFloatLogOnly();
      });
      recreated = isFloatWindowPairAlive();
    } catch (e4) {
      recreated = false;
    }
  }
  if (recreated) {
    ...
    try {
      appendWatchdogDiagLog("OPPO悬浮窗重建失败");
    } catch (e5) {}
  }
  return recreated;
}

function maybeOppoHardRecoveryFromWatchdog(silentMs, reason) {
  if (!isOppoAggressiveRecoveryEnabled()) return;
  if (__oppoHardRecoveryInProgress || __stuckRestartInProgress || __insideStuckHandler) return;
  if (!__automationWorkerActive || __scriptUserStop) return;
  var now = Date.now();
  if (now - __oppoLastHardRecoveryAttemptMs < 45000) return;
  __oppoLastHardRecoveryAttemptMs = now;
  __oppoHardRecoveryInProgress = true;
  threads.start(function () {
    try {
      sleep(__OPPO_HARD_RECOVERY_WAIT_MS);
      if (__scriptUserStop || !__automationWorkerActive) return;
      if (Date.now() - __lastAppendLogTimeMs < silentMs) return;
      if (__stuckRestartInProgress || __insideStuckHandler) return;
      __skipFloatUiFlush = true;
      var rsn = __pendingStuckRestartReason || reason;
      __pendingStuckRestartReason = "";
      appendLogFast("OPPO看门狗强制恢复");
      tryStuckRestartForRepeatedOrSilent(rsn);
    } catch (eHr) {
      try {
        appendLogFast("OPPO强制恢复异常:" + eHr);
      } catch (eHr2) {}
    } finally {
      __skipFloatUiFlush = false;
      __oppoHardRecoveryInProgress = false;
    }
  });
}

/** OPPO：常规恢复也卡住时，强打断无障碍并重启抖音后再拉流程 */
function maybeOppoNuclearUnstickFromWatchdog() {
  if (!isOppoAggressiveRecoveryEnabled() || !__automationWorkerActive || __scriptUserStop) return;
  var now = Date.now();
  var silentMs = now - __lastAppendLogTimeMs;
  var restartStuckMs =
    __stuckRestartInProgress && __stuckRestartStartedMs > 0
      ? now - __stuckRestartStartedMs
      : 0;
  var needNuclear =
    restartStuckMs >= 90000 ||
    silentMs >= 120000 ||
    (!isAccessibilityServiceAlive() && silentMs >= 50000);
  if (!needNuclear) return;
  if (now - __oppoLastNuclearUnstickMs < 60000) return;
  __oppoLastNuclearUnstickMs = now;
  threads.start(function () {
    try {
      __skipFloatUiFlush = true;
      __stuckRestartInProgress = false;
      __insideStuckHandler = false;
      __stuckRestartStartedMs = 0;
      __pendingStuckRestartReason = "";
      appendWatchdogDiagLog("OPPO强打断恢复");
      ...
      launchDouyin();
      sleep(2000);
      appendLogFast("强打断后重启抖音");
      tryStuckRestartForRepeatedOrSilent("过久无日志，重启脚本获取新对标");
    } catch (eN) {
      try {
        appendLogFast("强打断异常:" + eN);
      } catch (eN2) {}
    } finally {
      __skipFloatUiFlush = false;
    }
...
function applyAppendLogSideEffects(s) {
  function shouldSkipRepeatRestartForLog(txt) {
    ...
    if (txt.indexOf("非指定时段") === 0) return true;
    if (txt === "进入在线时段，继续运行") return true;
    if (txt === "进入指定时段，继续运行") return true;
    if (/停留 剩余 \d+ 秒$/.test(txt)) return true;
    if (txt.indexOf("无障碍") >= 0) return true;
    return false;
  }
  var now = Date.now();
...
function appendLog(msg) {
  ...
  pushLogLine(s);
  if (__skipFloatUiFlush) return;
  if (shouldOppoAutomationAsyncLogOnly()) {
    appendLogFloatFlushAsync();
    return;
  }
  if (s.indexOf("进入：") === 0) {
...
function flushFloatLogBlockingForce(blockMs) {
  ...
  if (isOppoColorOsDevice() && __automationWorkerActive) {
    ...
    if (blockMs > 0) sleep(Math.min(blockMs, 20));
    return;
  }
  try {
    logView.setText(logText);
...
/** 正版第11步连跑：连续 N 人未能进入作品 → 重启获取新对标（不计重启次数） */
var OFFICIAL_FAN_LIST_STEP10_FAIL_RESTART_N = 6;
...
/** 最近一次进入粉丝主页时的 visitKey（第10步失败时补记，防坐标去重失效） */
let __lastFollowerVisitKey = "";
...
/** OPPO/ColorOS：脚本跨应用打开抖音等时的系统确认框（勾选「始终允许打开」+ 点「打开」） */
function isOppoCrossAppOpenDialogVisible(appLabel) {
  appLabel = appLabel || "抖音";
  try {
    if (textContains("想要打开").findOne(80)) return true;
    if (textContains("始终允许打开").findOne(80)) return true;
    if (textContains("始终允许").findOne(80) && text("打开").findOne(80)) return true;
  } catch (e0) {}
  try {
    if (textContains(appLabel).findOne(80) && text("打开").findOne(80)) return true;
  } catch (e1) {}
  return false;
}

function clickOppoAlwaysAllowOpenCheckbox() {
  var target = null;
  try {
    target = textContains("始终允许打开").findOne(350);
  } catch (e0) {}
  if (!target) {
    try {
      target = textContains("始终允许").findOne(280);
    } catch (e1) {}
  }
  if (!target) {
    try {
      target = descContains("始终允许").findOne(220);
    } catch (e2) {}
  }
  ...
  try {
    var n = target;
    var pi;
    for (pi = 0; pi < 5 && n; pi++, n = n.parent()) {
      try {
        if (n.clickable && n.clickable() && n.click()) return true;
      } catch (eCl) {}
      try {
        if (n.className && String(n.className()).indexOf("CheckBox") >= 0 && n.click()) return true;
      } catch (eCb) {}
    }
    var b = target.bounds();
...
function confirmOppoCrossAppOpenDialog() {
  try {
    var coll = text("打开").find();
    if (coll && coll.size) {
      var i;
      for (i = 0; i < coll.size(); i++) {
        var btn = coll.get(i);
        try {
          if (String(btn.text() || "").trim() !== "打开") continue;
          var bb = btn.bounds();
          if (bb.width() < 24 || bb.height() < 18) continue;
          if (btn.clickable && btn.clickable() && btn.click()) return true;
          clickNode(btn);
          return true;
        } catch (eB) {}
      }
  ...
  try {
    var one = text("打开").findOne(400);
    if (one) {
      if (one.clickable && one.clickable() && one.click()) return true;
      clickNode(one);
      return true;
    }
  } catch (e1) {}
...
function handleOppoCrossAppOpenDialogIfAny(appLabel, maxMs) {
  if (!isOppoColorOsDevice()) return false;
  appLabel = appLabel || "抖音";
  maxMs = typeof maxMs === "number" && maxMs > 0 ? maxMs : 8000;
  var t0 = Date.now();
  while (Date.now() - t0 < maxMs && !__scriptUserStop) {
    if (!isOppoCrossAppOpenDialogVisible(appLabel)) {
      sleep(180);
      continue;
    }
    clickOppoAlwaysAllowOpenCheckbox();
    sleep(120);
    if (confirmOppoCrossAppOpenDialog()) {
      appendLog("已允许打开" + appLabel);
      sleep(400);
      return true;
    }
    sleep(200);
...
/** OPPO/ColorOS：首次运行「六道需要读取应用列表」系统权限框 → 点「允许」 */
function isOppoAppListPermissionDialogVisible() {
  try {
    if (textContains("需要读取应用列表").findOne(80)) return true;
    if (textContains("读取应用列表").findOne(80)) return true;
  } catch (e0) {}
  try {
    if (
      textContains("应用列表").findOne(80) &&
      text("允许").findOne(80) &&
      text("拒绝").findOne(80)
    ) {
      return true;
...
function confirmOppoAppListPermissionAllow() {
  var pkgs = [
  ...
  for (pi = 0; pi < pkgs.length; pi++) {
    try {
      var coll = text("允许").packageName(pkgs[pi]).find();
      if (coll && coll.size) {
        var best = null;
        var bestX = -1;
        var i;
        for (i = 0; i < coll.size(); i++) {
          var btn = coll.get(i);
          try {
            if (String(btn.text() || "").trim() !== "允许") continue;
            var bb = btn.bounds();
            if (bb.width() < 20 || bb.height() < 16) continue;
            if (bb.centerX() > bestX) {
              bestX = bb.centerX();
              best = btn;
            }
        ...
        if (best) {
          if (best.clickable && best.clickable() && best.click()) return true;
          clickNode(best);
          return true;
        }
      }
  ...
  try {
    ...
    if (all && all.size) {
      ...
      for (j = 0; j < all.size(); j++) {
        var btn2 = all.get(j);
        try {
          if (String(btn2.text() || "").trim() !== "允许") continue;
          var bb2 = btn2.bounds();
          if (bb2.width() < 20 || bb2.height() < 16) continue;
          if (bb2.centerX() > bestX2) {
            bestX2 = bb2.centerX();
            best2 = btn2;
          }
        } catch (eB2) {}
      ...
      if (best2) {
        if (best2.clickable && best2.clickable() && best2.click()) return true;
        clickNode(best2);
        return true;
      }
    }
  ...
  try {
    var ids = ["android:id/button2", "android:id/button1", "android:id/button3"];
    var ii;
    for (ii = 0; ii < ids.length; ii++) {
      ...
      tx = tx ? String(tx) : "";
      if (tx.indexOf("拒绝") >= 0 || /deny|decline/i.test(tx)) continue;
      if (/允许|Allow/i.test(tx)) {
...
/** 抖音搜索页：系统/厂商「添加至桌面」小组件弹窗 → 点「取消」 */
function isDouyinAddWidgetToDesktopDialogVisible() {
  try {
    if (textContains("添加至桌面").findOne(80)) return true;
    if (textContains("添加到桌面").findOne(80)) return true;
  } catch (e0) {}
  try {
    if (
      textContains("抖音搜索").findOne(80) &&
      text("取消").findOne(80) &&
      text("添加").findOne(80)
    ) {
      return true;
...
function dismissDouyinAddWidgetToDesktopDialogIfAny() {
  if (!isDouyinAddWidgetToDesktopDialogVisible()) return false;
  try {
    var coll = text("取消").find();
    if (coll && coll.size) {
      var i;
      for (i = 0; i < coll.size(); i++) {
        var btn = coll.get(i);
        try {
          if (String(btn.text() || "").trim() !== "取消") continue;
          var bb = btn.bounds();
          if (bb.width() < 20 || bb.height() < 16) continue;
          if (btn.clickable && btn.clickable() && btn.click()) {
            appendLog("已关闭添加桌面弹窗");
            sleep(320);
            return true;
          }
  ...
  try {
    var one = text("取消").findOne(350);
    if (one) {
      if (one.clickable && one.clickable() && one.click()) {
        appendLog("已关闭添加桌面弹窗");
        sleep(320);
        return true;
      }
...
function handleOppoAppListPermissionDialogIfAny(maxMs) {
  if (!isOppoColorOsDevice()) return false;
  maxMs = typeof maxMs === "number" && maxMs > 0 ? maxMs : 8000;
  var t0 = Date.now();
  while (Date.now() - t0 < maxMs && !__scriptUserStop) {
    if (!isOppoAppListPermissionDialogVisible()) {
      sleep(180);
      continue;
    }
    if (confirmOppoAppListPermissionAllow()) {
      appendLog("已允许读取应用列表");
      sleep(400);
      return true;
    }
...
function launchDouyin() {
  appendLog("开抖音…");
  handleOppoAppListPermissionDialogIfAny(3500);
  let ok = false;
  try { ok = app.launchApp("抖音"); } catch (e) {}
  if (!ok) {
    try { ok = app.launchPackage(DY_PKG); } catch (e) {}
  }
  handleOppoAppListPermissionDialogIfAny(4000);
  handleOppoCrossAppOpenDialogIfAny("抖音", 8000);
...
function launchSodaApp() {
  ...
  if (ok) {
    handleOppoCrossAppOpenDialogIfAny(SODA_APP_NAME, 8000);
    waitSodaSplashAdDismissed(SODA_SPLASH_ABSOLUTE_MAX_MS);
  }
  return ok;
}
...
function getNicknameFromMePage() {
  ...
  try {
    // 旧机型固定区域 + OPPO A72 / 华为（昵称 top≈284~312）
    var node =
      className("android.widget.TextView").boundsInside(360, 260, 980, 480).findOne(450) ||
      className("android.widget.TextView").boundsInside(408, 374, 718, 462).findOne(280);
    var fast = pickNickText(node);
    if (fast) return fast;
  } catch (e1) {}
  try {
    var sw = device.width;
    var sh = device.height;
    var xMin = Math.floor(sw * 0.18);
    var xMax = Math.floor(sw * 0.95);
    var yMin = Math.floor(sh * 0.1);
    var yMax = Math.floor(sh * 0.46);
    var coll = className("android.widget.TextView").find();
    if (coll && coll.size && coll.size() > 0) {
      var best = null;
      var bestTop = 1e9;
      for (var i = 0; i < coll.size(); i++) {
...
function clickSearchIcon() {
  appendLog("点搜索…");
  dismissDouyinAddWidgetToDesktopDialogIfAny();
...
function normalizeFollowerNickKey(nick) {
  var s = normalizeFollowerNicknameForLog(nick) || String(nick || "").trim();
  return s.replace(/\s+/g, " ").trim();
}

function resolveOfficialFollowerRowNickname(node, l0, t0, r0, b0, nickHint) {
  var nick = nickHint ? normalizeFollowerNickKey(nickHint) : "";
  if (!nick && node) {
...
function markFollowerRowVisitedKey(vk) {
  if (!vk) return;
  __followerVisitedNicks[vk] = 1;
  __lastFollowerVisitKey = vk;
}

/** 第10步失败：按抖音号或最近 visitKey 记为已访问，避免下滑后重复进同一人 */
function markFollowerVisitFromCurrentProfile() {
  try {
    var dyid = normalizeDbLine(readDouyinIdFromFanProfilePage());
    if (dyid) {
      markFollowerRowVisitedKey("dyid:" + dyid);
      return;
    }
  } catch (e0) {}
  if (__lastFollowerVisitKey) markFollowerRowVisitedKey(__lastFollowerVisitKey);
}

/** 去重用：优先昵称/抖音号，否则用行位置粗粒度桶（下滑后坐标会变） */
function followerRowVisitKey(node, nick) {
  var k = normalizeFollowerNickKey(nick);
  if (k) return k;
  try {
    var b = node.bounds();
    var slop = Math.max(36, Math.floor(device.height * 0.045));
    return "pos_" + Math.floor(b.top / slop) + "_" + Math.floor(b.left / slop);
  } catch (e) {
    return "";
  }
}
...
function collectFollowerListRowsDeduped() {
  ...
  var raw = [];
  var sw = device.width;
  var nodes1xt = dyUiFindCollection(function () {
    return id("com.ss.android.ugc.aweme:id/1xt").find();
  }, 1800);
  for (var i = 0; i < nodes1xt.length; i++) {
    var w = nodes1xt[i];
    try {
      var b = w.bounds();
      var cy = b.centerY();
  ...
  if (raw.length === 0) {
    try {
      var probeMs = isOppoColorOsDevice() ? 100 : 220;
      var one = id("com.ss.android.ugc.aweme:id/1xt").findOne(probeMs);
      if (one) raw.push(one);
    } catch (e1) {}
  }
  // 新 UI：列表行可能是全宽 Button，id=root_layout，无 1xt
  if (raw.length === 0) {
    var nodesRl = dyUiFindCollection(function () {
      return id("com.ss.android.ugc.aweme:id/root_layout").find();
    }, 1800);
    for (var j = 0; j < nodesRl.length; j++) {
      var rj = nodesRl[j];
      try {
        var br = rj.bounds();
        var cy2 = br.centerY();
...
function isFollowerListEffectivelyEmpty() {
  ...
  try {
    ...
    for (var ri = 0; ri < nodesRl.length; ri++) {
      var rn = nodesRl[ri];
      try {
        var br = rn.bounds();
        var cy = br.centerY();
  ...
  if (rows.length === 0) {
    try {
      var layNodes = dyUiFindCollection(function () {
        try {
          return className("android.widget.LinearLayout").packageName(DY_PKG).find();
        } catch (ep) {
          return className("android.widget.LinearLayout").find();
        }
      }, 1500);
      for (var i = 0; i < layNodes.length; i++) {
        var n = layNodes[i];
        try {
          var b = n.bounds();
          var cy = b.centerY();
...
function sodaFollowerRowAfterOpenWaitMs(logPrefix) {
  ...
  if (isHuaweiMate9Device()) return 900;
  return PACE_9_11.step9AfterOpenProfile;
}

function getOfficialProfileWorksLoadWaitMs() {
  if (isHuaweiMate9Device()) return 1200;
  return typeof WORK_GRID_PROFILE_LOAD_WAIT_MS === "number" && WORK_GRID_PROFILE_LOAD_WAIT_MS > 0
...
function tryClickFollowerRowNode(node, l0, t0, r0, b0, logPrefix, nickHint, skipEnterLog) {
  ...
  if (!isSoda) {
    nick = resolveOfficialFollowerRowNickname(node, l0, t0, r0, b0, nickHint);
  }
  if (!node) return false;
  var xBias = isSoda
    ? (typeof SODA_FOLLOWER_ROW_TAP_X_BIAS === "number" ? SODA_FOLLOWER_ROW_TAP_X_BIAS : 0.44)
...
function clickNextFollowerInList() {
  ...
  for (var pass = 0; pass < maxScrollPasses; pass++) {
    ...
    for (var i = 0; i < rows.length; i++) {
      var cand = rows[i];
      var nick = resolveOfficialFollowerRowNickname(cand, l0, t0, r0, b0, "");
      var vk = followerRowVisitKey(cand, nick);
      if (!vk) continue;
      if (__followerVisitedNicks[vk]) continue;
      if (tryClickFollowerRowNode(cand, l0, t0, r0, b0, "进入：", nick)) {
        markFollowerRowVisitedKey(vk);
        __noWorkLikeEntryDidBack = false;
        tryCaptureAndUploadFanProfileDouyinId();
        return true;
      }
...
function ensureDouyinProfileWorksTab() {
  var mate9Slow = isHuaweiMate9Device();
  ...
  function waitWorksGridBriefly(maxMs) {
    var endAt = Date.now() + (maxMs == null ? (mate9Slow ? 1800 : 700) : maxMs);
    while (Date.now() < endAt && !__scriptUserStop) {
      try {
        if (collectVisibleWorksGridContainers().length > 0) return true;
      } catch (eW0) {}
  ...
  try {
    ...
    if (waitWorksGridBriefly(mate9Slow ? 1800 : 700)) return true;
  } catch (eC0) {}
  try {
    if (isWorksTabSelected()) {
      if (!mate9Slow) return true;
      if (waitWorksGridBriefly(1200)) return true;
    }
  } catch (eC1) {}
  try {
    // Mate 9 等卡顿机：selected 态常读不到，作品网格未出时仍须点一次「作品」
    if (!mate9Slow && !selectedNonWorksTab()) return true;
  } catch (eC2) {}
  try {
    ...
    sleepCtrl(mate9Slow ? 720 : 360);
    if (mate9Slow) waitWorksGridBriefly(1500);
    return true;
  } catch (e0) {}
  return false;
}
...
function tryEnterWorkWithProfileOperateGate(maxStep) {
  ...
  while (guard < 18 && !__scriptUserStop) {
    ...
    if (!entered && !__scriptUserStop && !__workEntryTerminalFail) {
      sleepCtrl(isHuaweiMate9Device() ? 1400 : 800);
      entered = randomClickWorkLikeHeartOrBack();
    }
    if (!entered) markFollowerVisitFromCurrentProfile();
    return entered;
  }
  return false;
}
...
function tryStuckRestartForRepeatedOrSilent(reason) {
  ...
  __stuckRestartStartedMs = Date.now();
  try {
    var __label = "";
  ...
  threads.start(function () {
    ...
    while (true) {
      ...
      if (uiTick >= 10) {
        uiTick = 0;
        try {
          if (!__scriptUserStop && __automationWorkerActive) {
            if (isOppoColorOsDevice()) updateFloatInfoDirect();
            else updateFloatInfo();
          }
        } catch (eUi) {}
      }
      if (isOppoColorOsDevice() && __automationWorkerActive && !__scriptUserStop) {
        if (isOppoAggressiveRecoveryEnabled() && __oppoWatchdogSec % 30 === 0) {
          try {
            maybeOppoAccessibilityWatchdog();
          } catch (eA11y) {}
        }
        try {
          refreshOppoAutomationKeepAwake();
        } catch (eKa) {}
        if (__oppoWatchdogSec % 12 === 0) {
          try {
            ensureFloatWindowIfMissing("OPPO悬浮窗丢失补建");
          } catch (eEns) {}
        }
        if (__oppoWatchdogSec % 6 === 0) {
          try {
            maybeRecoverFrozenFloatWindow("OPPO悬浮窗冻结恢复");
          } catch (eFrz) {}
        }
        if (__oppoWatchdogSec % 20 === 0) {
          try {
            maybeProactiveRecreateFloatWindow();
          } catch (ePro) {}
        }
        if (__oppoWatchdogSec % 60 === 0) {
          try {
            persistAutomationRunHeartbeat();
          } catch (eHb2) {}
        }
        if (__oppoWatchdogSec % __OPPO_HEARTBEAT_EVERY_SEC === 0) {
          try {
            appendWatchdogDiagLog(
              "运行心跳 " +
                formatFloatNowDateTimeZh() +
                " 已运行" +
                formatRunDurationHoursMinutesZh(getRunTimeMsTodayForDisplay())
            );
          } catch (eHb) {}
        }
      }
      ...
      if (__quotaIdleKeepAwakeActive) continue;
      if (__insideStuckHandler || __stuckRestartInProgress) continue;
      var silentMs = isOppoAggressiveRecoveryEnabled()
        ? 45000
        : __STUCK_RESTART_SILENT_MS;
      if (Date.now() - __lastAppendLogTimeMs >= silentMs) {
        var reason = "过久无日志，重启脚本获取新对标";
        requestStuckRestart(reason);
        maybeOppoHardRecoveryFromWatchdog(silentMs, reason);
        maybeOppoNuclearUnstickFromWatchdog();
      }
    }
  });
}
...
/**
 * 正版抖音：分享/点赞等返回×2 后，校验是否回到对标粉丝列表（OPPO 上易停在粉丝用户主页）。
 */
function ensureDouyinFanListAfterWorkExit() {
  if (isSodaPlatformSelected() || __scriptUserStop) return;
  sleepCtrl(280);
  try {
    if (collectFollowerListRowsDeduped().length > 0) return;
    if (isLikelyOnFanUserProfilePageQuick()) {
      appendLog("作品返回后仍在粉丝主页，再返回");
      try {
        back();
      } catch (e0) {}
      sleepCtrl(450);
    ...
    tryEnsureBackToFanListIfNoFollowerRowsVisible();
    if (collectFollowerListRowsDeduped().length > 0) return;
    var hasFollowTab = false;
    var hasFansTab = false;
    try {
      hasFollowTab =
        !!dyUiFindOne(function () {
          return textMatches(/^关注\s*\d*$/).findOne(80);
        }, 900) ||
        !!dyUiFindOne(function () {
          return text("关注").findOne(80);
        }, 900);
      hasFansTab =
        !!dyUiFindOne(function () {
          return textMatches(/^粉丝\s*\d*$/).findOne(80);
        }, 900) ||
        !!dyUiFindOne(function () {
          return text("粉丝").findOne(80);
        }, 900);
    } catch (e2) {}
    if (!(hasFollowTab && hasFansTab) && isLikelyOnFanUserProfilePageQuick()) {
      appendLog("未回到对标粉丝列表，再返回");
      try {
        back();
      } catch (e3) {}
      sleepCtrl(520);
...
function pressBackTwiceAfterWork(act) {
  try {
    ...
    if (!isSoda) ensureDouyinFanListAfterWorkExit();
  } catch (e) {}
}
...
function runRandomSingleWorkInteraction(skipPostStep11FanListRestart) {
  ...
  if (!isSodaPlatformSelected()) appendLogFast("返回后继续");
  if (ok) {
    try {
      if (isSodaPlatformSelected()) {
...
/** 正版：第11步连跑连续进不了作品 → 重启获取新对标（不计入任务「重启」次数） */
function officialRestartForFanListStep10Stuck(maxStep) {
  if (__scriptUserStop || isTaskDailyQuotaComplete()) return false;
  var cap =
    typeof OFFICIAL_FAN_LIST_STEP10_FAIL_RESTART_N === "number" &&
    OFFICIAL_FAN_LIST_STEP10_FAIL_RESTART_N > 0
      ? Math.floor(OFFICIAL_FAN_LIST_STEP10_FAIL_RESTART_N)
      : 6;
  appendLog("连续" + cap + "人未能进入作品，重启获取新对标(不计重启次数)");
  try {
    __followerVisitedNicks = {};
  } catch (eClr) {}
  __lastFollowerVisitKey = "";
  __noWorkLikeEntryDidBack = false;
  return runFlowSteps1Through9ForInjectedRestart(maxStep, false);
}
...
function runFollowerListRepeatSteps10And11(maxStep) {
  ...
  while (safety < maxExtra) {
    ...
    if (!nextOk) {
      appendLog("粉丝列表下滑满次仍无新行，重启获取新对标(不计重启次数)");
      if (officialRestartForFanListStep10Stuck(maxStep)) {
        return processed;
      }
      break;
    }
    if (maxStep >= 10) {
      ...
      if (maxStep >= 11 && ok10) {
        ...
        markFollowerVisitFromCurrentProfile();
        try {
          if (collectFollowerListRowsDeduped().length === 0 && isLikelyOnFanUserProfilePageQuick()) {
            back();
            sleepCtrl(520);
        ...
        step10FailStreak++;
        appendLog("未能进入作品，换下一个(" + step10FailStreak + "/" + failRestartCap + ")");
        if (step10FailStreak >= failRestartCap) {
          if (officialRestartForFanListStep10Stuck(maxStep)) {
            return processed;
          }
          break;
...
function createOrShowFloatWindow() {
  ...
  if (__floatInfoWin && __floatCtrlWin) {
    ...
    updateAppVersionTitleDisplay();
    markFloatClockUiGood();
    if (!__oppoLastProactiveFloatRecreateMs) __oppoLastProactiveFloatRecreateMs = Date.now();
    startAutomationStuckWatchdogOnce();
    try {
      startOppoEmergencyStopKeyOnce();
    } catch (eKeyEarly) {}
    return;
  }
  ...
  let downX = 0, downY = 0;
  let movedFar = false;
  function bindFloatCloseHandlers() {
    if (!__floatCtrlWin || !__floatCtrlWin.btn_close) return;
    try {
      __floatCtrlWin.btn_close.setClickable(true);
    } catch (eClk) {}
    // click 兜底：部分 ColorOS 触摸分发异常时 touch 无回调，click 仍可触发
    try {
      if (__floatCtrlWin.btn_close.on) {
        __floatCtrlWin.btn_close.on("click", function () {
          requestStopFromFloatCloseTouch();
        });
      }
    } catch (eOn) {}
    try {
      __floatCtrlWin.btn_close.setOnClickListener({
        onClick: function () {
          requestStopFromFloatCloseTouch();
        },
      });
...
function showFloatWindow() {
  ...
  threads.start(() => {
    try {
      if (!__logLines.length && restoreAutomationLogSnapshot()) {
        appendWatchdogDiagLog("已恢复上次运行日志");
      }
    } catch (eRestore) {}
    createOrShowFloatWindow();
  });
...
if (ui.btn_start) {
  function __startAfterLicenseAuthorized() {
    ...
    toast("已显示悬浮框");
    try {
      checkAndLogAbnormalLastAutomationExit();
    } catch (eAbExit) {}
    if (isOppoColorOsDevice()) {
      try {
        appendLog(
          isOppoAggressiveRecoveryEnabled() ? "OPPO深度加固已启用" : "OPPO基础加固已启用"
        );
      } catch (eOppoHint) {}
    }
    appendLog("开始");
    ...
    threads.start(() => {
      ...
      try {
        handleOppoAppListPermissionDialogIfAny(8000);
      } catch (eOppoAppList0) {}
      try {
        beginOppoAutomationKeepAwake();
      } catch (eOaX0) {}
        ...
        try {
          ...
          try { endOppoAutomationKeepAwake(); } catch (eOaX) {}
          try { persistAutomationLogSnapshot(true); } catch (eLpX) {}
    ...
    threads.start(() => {
      ...
      try {
        ...
        if (isTaskDailyQuotaComplete()) {
          ...
          try {
            ...
            for (qj = 0; qj < 4 && !__scriptUserStop; qj++) {
              ...
              __quotaIdleLoopTick++;
              try {
                maybeRecoverFrozenFloatWindow("满额等待悬浮窗自检");
              } catch (eQf) {}
              if (__quotaIdleLoopTick % 40 === 0) {
                try {
                  appendWatchdogDiagLog(
                    "满额等待零点 " +
                      formatFloatNowDateTimeZh() +
        ...
        try { markAutomationRunCleanStop(); } catch (eMkFin) {}
        try { endQuotaIdleKeepAwake(); } catch (eQ3) {}
        try { endOppoAutomationKeepAwake(); } catch (eOa1) {}
        try { persistAutomationLogSnapshot(true); } catch (eLp1) {}
        __automationWorkerActive = false;
